...
### 自動プッシュルール
- **コード・コンテンツ・設定ファイルへの変更が完了したら、毎回必ず `git add → commit → push` を行うこと。**
- ユーザーから「プッシュして」と言われなくても、作業完了時に自動でプッシュする。
- コミットメッセージは本ファイルのプレフィックス規則（`feat:` `fix:` `content:` 等）に従う。
- `other/` ディレクトリは `.gitignore` 対象のため、絶対にステージングしない。
